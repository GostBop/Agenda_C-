...
void Meeting::removeParticipator(const std::string &t_participator) {
  auto index = std::find(m_participators.begin(), m_participators.end(), t_participator);
  if(index != m_participators.end())
    m_participators.erase(index);
}
...
bool Meeting::isParticipator(const std::string &t_username) const {
  auto index = std::find(m_participators.begin(), m_participators.end(), t_username);
  if(index != m_participators.end())
    return true;
  else
   return false;
